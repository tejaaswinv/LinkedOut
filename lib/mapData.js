...
export function mapCompanyRow(row) {
  ...
  return {
    ...
    revenueUsd: row.revenue_usd == null ? null : Number(row.revenue_usd),
    rankingProvider: row.ranking_provider || null,
    rankingYear: row.ranking_year || null,
    rankingPosition: row.ranking_position || null,
    rankingSourceUrl: row.ranking_source_url || null,
    ...
    reviews,
    employees,
    rank: row.ranking_position ? `${row.ranking_provider || 'Global'} #${row.ranking_position}` : (reviews ? 'Based on LinkedOut reviews' : 'New page'),
    trend: 0,
    metrics,
    issues,
