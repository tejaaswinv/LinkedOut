...
export async function POST(request) {
  ...
  catch { return NextResponse.json({ error: 'Enter the 6-digit verification code.' }, { status: 400 }); }

  const { data: row } = await admin.from('employment_verifications')
    .select('*')
  ...
  if (!safeEqual(expected, row.code_hash)) {
    await admin.from('employment_verifications').update({ attempts: row.attempts + 1 }).eq('id', row.id);
