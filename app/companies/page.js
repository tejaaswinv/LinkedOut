...
export default function Companies(){
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);if(params.get('q'))setQ(params.get('q'));fetch('/api/companies?limit=100').then(r=>r.json()).then(d=>{if(d.companies?.length)setCompanies(d.companies)}).catch(()=>{})},[]);
