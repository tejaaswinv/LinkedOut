...
export default function Profile(){
  ...
  const [form,setForm]=useState({username:'',bio:'',position:'',department:'',location:'',employmentStatus:'current',currentCompanyId:'',currentUniversityId:'',fieldOfStudy:'',graduationYear:'',showCompany:true,showPosition:true,showDepartment:false,showLocation:true,showUniversity:true,showFieldOfStudy:true,showGraduationYear:false});
  ...
  const load=async()=>{
    const [r,companyResponse,universityResponse]=await Promise.all([authFetch('/api/profile'),fetch('/api/companies?limit=100'),fetch('/api/universities?limit=150')]);
    const [d,companyData,universityData]=await Promise.all([r.json(),companyResponse.json(),universityResponse.json()]);
  ...
  return <><Nav/><main className="pageWrap narrow"><div className="card profilePage"><div className="profileCover tall"></div><div className="avatar profileHero">◕{emailVerified&&<span className="verifyDot">✓</span>}</div><h1>{form.username}</h1><div className="verified">{emailVerified?'Verified account':'Email verification pending'}</div>
    ...
    <form className="profileEdit" onSubmit={save}>
      ...
      <div className="visibilityList compact">{isStudent?<><PrivacyToggle title="Show university" checked={form.showUniversity} onChange={value=>setForm({...form,showUniversity:value})}/><PrivacyToggle title="Show field of study" checked={form.showFieldOfStudy} onChange={value=>setForm({...form,showFieldOfStudy:value})}/><PrivacyToggle title="Show graduation year" checked={form.showGraduationYear} onChange={value=>setForm({...form,showGraduationYear:value})}/></>:<><PrivacyToggle title="Show company" checked={form.showCompany} onChange={value=>setForm({...form,showCompany:value})}/><PrivacyToggle title="Show position" checked={form.showPosition} onChange={value=>setForm({...form,showPosition:value})}/><PrivacyToggle title="Show department" checked={form.showDepartment} onChange={value=>setForm({...form,showDepartment:value})}/></>}<PrivacyToggle title="Show location" checked={form.showLocation} onChange={value=>setForm({...form,showLocation:value})}/></div>
