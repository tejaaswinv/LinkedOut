...
export default function RightRail({initialCompanies}) {
  ...
  useEffect(()=>{if(initialCompanies?.length){setCompanies(initialCompanies);return;}fetch('/api/companies?limit=8').then(r=>r.json()).then(d=>{setCompanies(Array.isArray(d.companies)?d.companies:[])}).catch(()=>setCompanies([]))},[initialCompanies]);
  ...
  const visible=ranked.length?ranked:companies.slice(0,5);
  return <aside className="rightcol">
    <div className="card railCard"><div className="railTitle">🔥 Trending Companies <Link href="/companies">View all</Link></div>{visible.length?visible.map((c,i)=><Link href={`/company/${c.slug}`} className="trendRow" key={c.slug}><b>{i+1}</b><span className={`tinyLogo logo-${c.slug}`}>{c.logo}</span><span>{c.name}</span><strong>{c.reviews>0?Number(c.score||0).toFixed(1):'new'} <i>{c.trend>0?'↑':c.trend<0?'↓':''}</i></strong></Link>):<small>No community activity yet.</small>}<small>Recognized pages + community review activity</small></div>
    ...
    <div className="card railCard"><div className="railTitle">Most Discussed <Link href="/companies">View all</Link></div>{companies.length?companies.slice().sort((a,b)=>b.reviews-a.reviews).slice(0,5).map((c,i)=><Link href={`/company/${c.slug}`} className="trendRow" key={c.slug}><b>{i+1}</b><span className={`tinyLogo logo-${c.slug}`}>{c.logo}</span><span>{c.name}</span><em>{c.reviews?c.reviews.toLocaleString():'New'}</em></Link>):<small>No published discussions yet.</small>}</div>
  </aside>;
}
